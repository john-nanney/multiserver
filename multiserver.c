...
void usage(void)
{
	...
	printf("SERVER_PORT is the block map server port, defaults to %d\n",
	       DEFAULT_SERVER_PORT);
	printf
	    ("INTERPACKET_DELAY is the delay in milliseconds between multicast packets. This is only for debugging, never actually use it.\n");
	puts("\n");
	exit(EXIT_FAILURE);
}
...
void setup_map_packet(uint64_t filesize, uint32_t blocksize,
		      const char *filename)
{
	map_packet_payload.filesize = htobe64(filesize);
	map_packet_payload.blocksize = htobe32(blocksize);
	/* Trailer block if uneven. */
	map_packet_payload.nblocks =
	    htobe32((filesize / blocksize) + (filesize % blocksize ? 1 : 0));
	...
	map_packet_header.size =
	    htobe16((uint16_t)
		    (sizeof(map_packet_payload) + strlen(filename) + 1));
...
void *control_thread(void *arg)
{
	...
	while (1) {
		...
		if (rv) {
			...
			bytes =
			    recvfrom(ctlsock, &bdbuf, MAX_PACKET_SIZE, 0,
			...
			if (packet->header.magic != MXBP_MAGIC) {
				printf
				    ("Not a valid header, wanted 0x%08x got 0x%08x\n",
			...
			case MXBP_MAPREQ:
				...
				if (sendmsg(ctlsock, &mapmsg, 0) < 0) {
					printf
					    ("sendmsg(): failed sending map message: %s\n",
					     strerror(errno));
					exit(EXIT_FAILURE);
				}
				break;
			...
			case MXBP_BLOCKREQ:
				if (packet->header.size < sizeof(uint32_t) * 2) {
					printf
					    ("Got block request but not enough bytes to specify a range.\n");
					continue;
				}
				...
				if (!
				    (nextblockreq =
				     malloc(sizeof(blockdesc_t)))) {
					printf
					    ("Memory allocation failed for %ld bytes\n",
					     sizeof(blockdesc_t));
					exit(EXIT_FAILURE);
				}
				...
				if (firstblock > lastblock) {
					printf
					    ("Mangled block request: start %d end %d, ignoring.\n",
				...
				printf
				    ("Pushing range %d to %d onto the block range list.\n",
				     firstblock, lastblock);
			...
			default:
				printf
				    ("Unrecognized operation in control server: %d\n",
				     packet->header.op);
...
void data_server_thread(void)
{
	...
	if (!
	    (block_packet_buffer =
	     malloc(sizeof(mxbp_header_t) + MXBP_BLOCKSIZE))) {
	...
	while (1) {
		...
		if (!qtop) {
			...
			if (pthread_cond_timedwait(&wqready, &wqlock, &ts)) {
				if (errno && errno != ETIMEDOUT) {
					printf
					    ("Waiting for work items failed: %s (%d)\n",
		...
		if (start) {
			if (lseek64
			    (filefd, (uint64_t) start * MXBP_BLOCKSIZE,
			     SEEK_SET) == -1) {
		...
		for (x = start; x <= end; ++x) {
			bytes =
			    read(filefd, block_packet->data, MXBP_BLOCKSIZE);
			if (bytes == -1) {
				printf("Error reading file: %s\n",
				       strerror(errno));
				if (++read_error_count > read_error_count_max) {
					printf
					    ("Too many failures (%d), bailing out.\n",
					     read_error_count);
					exit(EXIT_FAILURE);
				}
				continue;
			...
			if (sendto
			    (sock, block_packet,
			     sizeof(block_packet->header) + bytes, 0,
